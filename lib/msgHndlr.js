...
const pr_ = JSON.parse(fs.readFileSync('./lib/CH.json'))
const df_ = JSON.parse(fs.readFileSync('./lib/DF.json'))
const fn_ = JSON.parse(fs.readFileSync('./lib/FN.json'))
const ct_ = JSON.parse(fs.readFileSync('./lib/CT.json'))
...
module.exports = msgHandler = async (client, message, event) => {
    try {
        ...
        const apiKey = 'API-KEY' // apikey you can get it at http://mhankbarbars.herokuapp.com/api
        const RasiApi = 'rasitech' // apikey you can get it at https://vhtear.com/
        ...
        const ownerNumber = ["6287883353433@c.us","6287883353433"] // replace with your whatsapp number
        ...
        if (isGroupMsg && command.startsWith('!')) console.log('\x1b[1;37m|\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(msgs(command)), 'from', color(pushname), 'in', color(formattedTitle))
        //if (!isGroupMsg && !command.startsWith('!')) console.log('\x1b[1;33m~\x1b[1;37m>', '[\x1b[1;31mMSG\x1b[1;37m]', time, color(body), 'from', color(pushname))
        //if (isGroupMsg && !command.startsWith('!')) console.log('\x1b[1;33m~\x1b[1;37m>', '[\x1b[1;31mMSG\x1b[1;37m]', time, color(body), 'from', color(pushname), 'in', color(formattedTitle))
        if (!isGroupMsg && command.startsWith('')) fs.writeFile(`./akun/${from}.txt`, `Nama : ${pushname}\nNomor : ${from.replace(/@c.us/g, ' ')}\nPesan : ${args[1]}`)
        if ( isGroupMsg && command.startsWith('')) fs.writeFile(`./akun/${from}.txt`, `Nama : ${pushname}\nNomor : ${from.replace(/@g.us/g, ' ')}\nPesan : ${args[1]}`)
	...
	if(message.body == `${ltatas}`){
        ...
        case '!stiker':
            ...
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await client.sendFile(from, imageBase64, 'imagesticker.jpg', 'Success Convert Sticker to Image!', id)
...
const { exec } = require('child_process')
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            ...
                            get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0AMaaf filenya terlalu besar`).json() 
                        })
                    })
                  } else {
                    client.reply(from, `[❗] Kirim gif dengan caption *!stickergif*,*!stikergif* atau *!sgf* max 10 sec!`, id)
                   }
                } else {
		    client.reply(from, `[❗] Kirim gif dengan caption *!stickergif*,*!stikergif* atau *!sgf* max 10 sec!`, id)
	        }
            ...
            if (isMedia) {
                    ...
                    await fs.writeFileSync(filename, mediaData)
                        await client.sendFile(from, `data:${mimetype};base64,${mediaData.toString('base64')}`, `Rasi Tech Bot ${filename}`)
            ...
            case '!vts':
                ...
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 60) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    client.reply(from, '[WAIT] Sedang di proses⏳ silahkan tunggu ± 1 min!', id)
                    const filename = `./media/aswu.${mimetype.split('/')[1]}`
                    await fs.writeFileSync(filename, mediaData)
                        await client.sendAudio(from, `data:audio/mp3;base64,${mediaData.toString('base64')}`)
                } else (
                    client.reply(from, '[❗] Kirim video dengan caption *!vts* max 1 Menit!', id)
                )
            }
            break
            case '!vtv':
            ...
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 60) {
                    ...
                    const filename = `./media/aswu.${mimetype.split('/')[1]}`
                    await fs.writeFileSync(filename, mediaData)
                        await client.sendFile(from, `data:audio/mp3;base64,${mediaData.toString('base64')}`, `Rasi Tech Bot ${filename}`)
                } else (
                    client.reply(from, '[❗] Kirim video dengan caption *!vtv* max 1 Menit!', id)
                )
            }
            break
	    case '!stickernobg':
	    ...
	    if (isMedia) {
                try {
                    ...
                    var result = await removeBackgroundFromImageBase64({ base64img, apiKey: 'ei3o1CqgMtDPNwJJfDYJTUTf', size: 'auto', type: 'auto', outFile })
        ...
        case '!donate':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.reply(from, `Ya halo om.. Mau donate?
    
            Kalo mau donate langsung ae ke :
            OVO/PULSA/GOPAY : 085764424816
         
            Thanks !
            
            Dev : 087883353433 | https://wa.me/6287883353433`, donate)
            break
        case '!tts':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
        ...
        case '!nulis':
            ...
            let urlnulis = `http://mhankbarbars.herokuapp.com/nulis?text=${nulis}`
            await fetch(urlnulis, {method: "GET"})
            .then(res => res.json())
            .then(async (json) => {
                await client.sendFileFromUrl(from, json.result, 'Nulis.jpg', 'Nih...', id)
            }).catch(e => client.reply(from, "Error: "+ e));
            get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0AError: ${e}`).json() 
            break
            case '!nulis2':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!nulis2 [teks]*', id)
            const nulis2 = encodeURIComponent(body.slice(8))
            client.reply(from, mess.wait, id)
            let urlnulis2 = `https://api.i-tech.id/tools/nulis?key=3b8JTm-WeSnVm-ynZYWt-mnMT1e-n888YG&text=${nulis2}`
            await fetch(urlnulis2, {method: "GET"})
            .then(res2 => res2.json())
            .then(async (json2) => {
                await client.sendFileFromUrl(from, json2.result, 'Nulis2.jpg', 'Nih...', id)
            }).catch(e => client.reply(from, "Error: "+ e));
            get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0AError: ${e}`).json() 
            break
        case '!ytmp3':
            ...
            try {
                client.reply(from, mess.wait, id)
                const resp = await get.get(`http://mhankbarbars.herokuapp.com/api/yta?url=${args[1]}`).json()
                if (resp.error) {
                ...
                get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0AError ytmp3 : ${err}`).json() 
        ...
        case '!ytmp4':
            ...
            try {
                client.reply(from, mess.wait, id)
                const ytv = await get.get(`https://api.i-tech.id/dl/yt?key=3b8JTm-WeSnVm-ynZYWt-mnMT1e-n888YG&link=${args[1]}`).json()
                if (ytv.error) {
                    client.reply(from, ytv.error, id)
                } else {
                    if (Number(ytv.size.split(' MB')[0]) > 40.00) return client.reply(from, 'Maaf durasi video sudah melebihi batas maksimal!', id)
                    client.sendFileFromUrl(from, ytv.thumbnail, 'thumb.jpg', `➸ *Title* : ${ytv.title}\n➸ *Filesize* : ${ytv.size}\n➸ *Durasi* : ${ytv.duration}\n➸ *Detik* : ${ytv.second}\n➸ *Dekripsi* : ${ytv.desc}\n\nSilahkan tunggu sebentar proses pengiriman file membutuhkan waktu beberapa menit.`, id)
                    await client.sendFileFromUrl(from, ytv.url_video, `${ytv.title}.mp4`, '', id).catch(() => client.reply(from, mess.error.Yt4, id))
                }
            } catch (er) {
                client.sendText(ownerNumber[0], 'Error ytmp4 : '+ er)
                get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0AError ytmp4 : ${er}`).json() 
            ...
            case '!smule':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!smule [linkSmule]*, untuk contoh silahkan kirim perintah *!readme*')
            if (!args[1].includes('smule.com')) return client.reply(from, mess.error.Iv, id)
            try {
                client.reply(from, mess.wait, id)
                const smule = await get.get(`https://api.i-tech.id/dl/smule?key=3b8JTm-WeSnVm-ynZYWt-mnMT1e-n888YG&link=${args[1]}`).json()
                if (smule.error) {
                    client.reply(from, smule.error, id)
                } else {
                    client.sendFileFromUrl(from, smule.img, 'smule.jpg', `➸ *Title* : ${smule.title}\n\n➸ *Type* : ${smule.type}\n\nSilahkan tunggu sebentar proses pengiriman file membutuhkan waktu beberapa menit.`, id)
                    await client.sendFileFromUrl(from, smule.url, `${smule.title}.mp4`, '', id).catch(() => client.reply(from, mess.error.smule, id))
                }
            } catch (er) {
                client.sendText(ownerNumber[0], 'Error ytmp4 : '+ er)
                get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0AError ytmp4 : ${er}`).json() 
        ...
        case '!wiki':
            ...
            const wiki = await get.get(`http://mhankbarbars.herokuapp.com/api/wiki?q=${query_}`).json()
            if (wiki.error) {
            ...
            case '!tts2':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!tts2 [query]*\nContoh : *!tts2 apa kabar kalian?*', id)
            client.sendPtt(from, `https://rt-files.000webhostapp.com/tts.php?apikey=rasitech&kata=${body.slice(6)}`, `tts2.mp3`, `Sudah jadi`,id)
            break
        case '!cuaca':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!cuaca [tempat]*\nContoh : *!cuaca tangerang', id)
            const tempat = body.slice(7)
            const weather = await get.get(`http://mhankbarbars.herokuapp.com/api/cuaca?q=${tempat}`).json()
            if (weather.error) {
        ...
        case '!fb':
            ...
            client.reply(from, mess.wait, id)
            const epbe = await get.get(`http://mhankbarbars.herokuapp.com/api/epbe?url=${args[1]}`).json()
            if (epbe.error) return client.reply(from, epbe.error, id)
            client.sendFileFromUrl(from, epbe.result, 'epbe.mp4', epbe.title, id)
            break
            case '!twitter':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!twitter [linkTwitter]* untuk contoh silahkan kirim perintah *!readme*', id)
            if (!args[1].includes('twitter.com')) return client.reply(from, mess.error.Iv, id)
            client.reply(from, mess.wait, id)
            const twitter = await get.get(`http://mhankbarbars.herokuapp.com/api/twit?url={args[1]}`).json()
            if (epbe.error) return client.reply(from, twitter.error, id)
            client.sendFileFromUrl(from, twitter.result, 'twitter.mp4', `Judul : ${twitter.title}\nFile Size : ${twitter.filesize}\nQuote : ${twitter.quote}`, id)
            break
            case '!tiktok':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!tiktok [linkTiktok]* untuk contoh silahkan kirim perintah *!readme*', id)
            if (!args[1].includes('www.tiktok.com')) return client.reply(from, mess.error.Iv, id)
            client.reply(from, mess.wait, id)
            const tiktok = await get.get(`https://api.vhtear.com/tiktokdl?link=${args[1]}&apikey=${RasiApi}`).json()
            if (tiktok.error) return client.reply(from, tiktok.error, id)
            client.sendFileFromUrl(from, tiktok.video, 'tiktok.mp4', `Rasi Tech Bot`, id)
            break
        case '!creator':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.sendContact(from, '6287883353433@c.us')
            break
            case '!nomorbot':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                let tanya = body.slice(9)
                let jum = Number(tanya.split('.')[1]) || 2
            client.reply(from, `*Robot Rasi Tech*\nhttps://wa.me/6285764424816`, id)
            client.sendVCard('625764424816c.us', 'string","Robot Rasi Tech","6285764424816' )
            break
        case '!ig':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            try {
                client.reply(from, mess.wait, id)
                const resp = await get.get(`https://api.i-tech.id/dl/igdl?key=3b8JTm-WeSnVm-ynZYWt-mnMT1e-n888YG&link=${args[1]}`).json()
            ...
            try {
                client.reply(from, mess.wait, id)
                const resp = await get.get(`http://mhankbarbars.herokuapp.com/api/ig?url=${args[1]}`).json()
                if (resp.result.includes('.mp4')) {
                    var ext = '.mp4'
                } else {
                    var ext = '.jpg'
        ...
        case '!nsfw':
            ...
            case '!gmail':
                ...
                arg = body.trim().split('|')
            if (arg.length >= 2) {
                const gemail = encodeURIComponent(arg[1])
                const pesannya = encodeURIComponent(arg[2])
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                "use strict";
                ...
                async function main() {
                  ...
                  let transporter = nodemailer.createTransport({
                    ...
                    auth: {
                      user: "hayley.mohr4@ethereal.email", // generated ethereal user
                      pass: "f3utC1USzQjeNVZaFv", // generated ethereal password
                    },
                  });
                
                  // send mail with defined transport object
                  let info = await transporter.sendMail({
                    from: 'hayley.mohr4@ethereal.email', // sender address
                    to: gemail, // list of receivers
                    subject: "Rasi Tech Bot", // Subject line
                    html: pesannya, // html body
                  });
                
                  console.log("Message sent: %s", info.messageId);
                  client.reply(form, "Message sent: %s", info.messageId)
                  // Message sent: <b658f8ca-6296-ccf4-8306-87d57a0b4321@example.com>
                
                  // Preview only available when sending through an Ethereal account
                  console.log("Preview URL: %s", nodemailer.getTestMessageUrl(info));
                  client.reply(form, "Preview URL: %s", nodemailer.getTestMessageUrl(info))
                  // Preview URL: https://ethereal.email/message/WaQKMgKddxQDoou...
                }
                client.reply(from, 'Masukan Seperti ini !gmail |email|pesan\n\n*!gmail |rasi@gmail.com|pesannya oi*', id)
                main().catch(console.error);
                get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0A${error}`).json() 
            ...
            if (args[2].toLowerCase() === 'enable') {
                pr_.push(`${args[1]}@c.us`)
                //pr_.push(`${args[1]}`)
                fs.writeFileSync('./lib/CH.json', JSON.stringify(pr_))
                client.reply(from, `Premium Command berhasil di aktifkan di Nomor ${args[1]}!`, id)
            } else if (args[2].toLowerCase() === 'disable') {
                //pr_.splice(`${args[1]}`, 1)
                pr_.splice(`${args[1]}@c.us`, 1)
                fs.writeFileSync('./lib/CH.json', JSON.stringify(pr_))
                client.reply(from, `Premium Command berhasil di nonaktifkan di Nomor ${args[1]}!`, id)
            } else {
                client.reply(from, 'Pilih enable atau disable udin!', id)
            }
            break
            case '!daftar':
                if (isDf) return client.reply(from, 'Anda sudah terdaftar!', id)
                if (isGroupMsg) return client.reply(from, `Perintah ini tidak bisa di group!`, id)
                df_.push(from)
                //pr_.push(`${args[1]}`)
                fs.writeFileSync('./lib/DF.json', JSON.stringify(df_))
                client.reply(from, `berhasil daftar di Nomor ${from.replace(/@c.us/g,'')}!`, id)
            break
            case '!post':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Masukan nama group! ex: Rasi Tech Group', id)
                const postg = await client.getGroupInviteLink(groupId);
                fs.writeFileSync('./lib/FN.json', JSON.stringify(fn_))
                fn_.push(`*${body.slice(6)}*\n\n${postg}`)
            break       
            case '!find':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                const efen = JSON.parse(fs.readFileSync('./lib/FN.json'))
                const found = efen.find(element => element > body.slice(6));
                client.reply(from, found, id)
                // expected output: 12
            break
            case '!18+':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
                const sex = await get.get(`http://mhankbarbars.herokuapp.com/api/indohot`).json()
                if (sex.error) return client.reply(from, sex.error, id)
                const caps2 = `➸ *Negara* : ${sex.country}\n➸ *Durasi* : ${sex.durasi}\n➸ *Genre* : ${sex.genre}\n➸ *Judul* : ${sex.judul}\n➸ *Link : ${sex.url}\n➸ *Rasi Tech Bot* : Rasi Tech`
        ...
        case '!tkstalk':
            ...
            const stalk = await get.get(`http://mhankbarbars.herokuapp.com/api/stalk?username=${args[1]}`).json()
            if (stalk.error) return client.reply(from, stalk.error, id)
            const { Bio, followers, following, post, pic,description,verified, like } = stalk
            const caps1 = `➸ *Nama* : ${stalk1.name}\n➸ *Username* : ${stalk1.username}\n➸ *Jumlah Followers* : ${followers}\n➸ *Jumlah Following* : ${following}\n➸ *Jumlah Postingan* : ${post}\n➸ *Deskripsi* : ${description}\n➸ *Terverifikasi* : ${verified}\n➸ *Like* : ${like}\n➸ *Biodata* : ${Bio}\n➸ *Link* : ${stalk1.url}`
            await client.sendFileFromUrl(from, pic, 'Profile1.jpg', caps1, id)
            break
            case '!igstalk':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1)  return client.reply(from, 'Kirim perintah *!tkStalk @username*\nConntoh *!igStalk @duar_amjay*', id)
            const stalk1 = await get.get(`http://mhankbarbars.herokuapp.com/api/stalk?username=${args[1]}`).json()
            if (stalk1.error) return client.reply(from, stalk1.error, id)
            const { Biodata, Jumlah_Followers, Jumlah_Following, Jumlah_Post, Name, Username, Profile_pic } = stalk1
            ...
            case '!twitstalk':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1)  return client.reply(from, 'Kirim perintah *!twitstalk username*\nConntoh *!twitstalk Mark*', id)
            const twitstalk = await get.get(`http://mhankbarbars.herokuapp.com/api/twstalk?username=${args[1]}`).json()
            if (stwitstalk.error) return client.reply(from, twitstalk.error, id)
            await client.sendFileFromUrl(from, twitstalk.profile_pic, 'twit.jpg', `➸ *Nama* : ${twitstalk.name}\n➸ *Nama Lengkap* : ${twitstalk.full_name}\n➸ *Jumlah Followers* : ${twitstalk.followers_count}\n➸ *ID* : ${twitstalk.id}\n➸ *Jumlah Status* : ${twitstalk.status_count}`, id)
            break
            case '!dewabatch':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1)  return client.reply(from, 'Kirim perintah *!dewabatch anime*\nConntoh *!dewabatch naruto*', id)
            const dewabatch = await get.get(`http://mhankbarbars.herokuapp.com/api/dewabatch?q=${body.slice(11)}`).json()
            if (dewabatch.error) return client.reply(from, dewabatch.error, id)
            await client.sendFileFromUrl(from, dewabatch.thumb, 'dewabatch.jpg', `➸ *Sinopsis* : ${dewabatch.sinopsis}\n➸ *Info* : ${dewabatch.result}`, id)
            break
            case '!kusonime':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length === 1)  return client.reply(from, 'Kirim perintah *!kusonime anime*\nConntoh *!kusonime naruto*', id)
            const kusonime = await get.get(`http://mhankbarbars.herokuapp.com/api/kuso?q=${body.slice(11)}`).json()
            if (kusonime.error) return client.reply(from, kusonime.error, id)
            await client.sendFileFromUrl(from, kusonime.thumb, 'kusonime.jpg', `➸ *Sinopsis* : ${kusonime.sinopsis}\n➸ *Info* : ${kusonime.info}\n➸ *Judul* : ${kusonime.title}\n➸ *Link Download* : ${kusonime.link_dl}`, id)
            break
            case '!komiku':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            if (args.length === 1)  return client.reply(from, 'Kirim perintah *!komiku anime*\nConntoh *!komiku naruto*', id)
            const komiku = await get.get(`http://mhankbarbars.herokuapp.com/api/kuso?q=${body.slice(11)}`).json()
            if (komiku.error) return client.reply(from, komiku.error, id)
            await client.sendFileFromUrl(from, komiku.thumb, 'komiku.jpg', `➸ *Sinopsis* : ${komiku.sinopsis}\n➸ *Info* : ${komiku.info}\n➸ *Genre* : ${komiku.genre}\n➸ *Link Download* : ${komiku.link_dl}`, id)
            break
            case '!catat':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (isGroupMsg && !isGroupAdmins ) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
                fs.writeFile(`./catat/${from}.txt`, body.slice(7))
                client.reply(from, `Berhasil Nyatet!`, id)
            break
            case '!catet':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.reply(from, `Salah! gunakan !catat tadi kamu bukan !catat tapi !catet . mungkin owner salah ketik saat broadcast. `, id)
            break
            case '!catatan':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                var catatan = fs.readFileSync(`./catat/${from}.txt`, 'utf8')
                client.reply(from, `[Catatan *${chat.contact.name}* ]\n\n${catatan}`, id)
            break
        case '!infogempa':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const bmkg = await get.get(`https://api.i-tech.id/tools/bmkg?key=3b8JTm-WeSnVm-ynZYWt-mnMT1e-n888YG`).json()
        ...
        case '!anime':
            ...
            const animek = await get.get(`http://mhankbarbars.herokuapp.com/api/kuso?q=${body.slice(7)}`).json()
        ...
        case '!wait':
            ...
            case '!urltrack':
                ...
                if (!args[1].match(isUrl)) return client.reply(from, mess.error.Iv, id) 
                    ...
                    body: JSON.stringify({"url": args[1]})
                })
                .then(respon => respon.json())
                .then(resolt => {
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                const fetch = require('node-fetch')
                const imgBS4 = `data:${mimetype};base64,${mediaData.toString('base64')}`
                client.reply(from, 'Uploading....', id)
                fetch(`https://api.vhtear.com/imageUrl&apikey=${RasiApi}`, {
                    method: 'POST',
                    body: JSON.stringify({ image: imgBS4 }),
                    headers: { "Content-Type": "application/json" }
                })
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		client.reply(from, 'Gagal upload mungkin di sebabkan oleh sistem.', id)
                	}
                    if (similarity < 0.92) {
                        client.reply(from, resolt.docs[0].url , id)
                    }
                })
                .catch(() => {
                    client.reply(from, 'Error !', id)
        ...
        case '!bci':
            ...
            if (arg.length >= 2) {
                client.reply(from, mess.wait, id)
                const bcp = encodeURIComponent(arg[2])
                const imgb = encodeURIComponent(arg[1])
            const chatzi = await client.getAllChatIds()
            for (let ids of chatzi) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) await client.sendFileFromUrl(ids, imgb , `[ Rasi Tech BOT Broadcast 087883353433 ]\n\n${bcp}`)
            }
        }
            ...
            for (let ids of chatz) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) await client.sendText(ids, `[ Rasi Tech BOT Broadcast 087883353433 ]\n\n${msg}`)
            }
            client.reply(from, 'Broadcast Success!', id)
            break
            case '!list':
        ...
        if (isGroupMsg) fs.writeFile(`./akun/${i}.txt`, `${i.replace(/@g.us/g,'')}`)
                ...
                case '!listpr':
                    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const listprp =  pr_
                let listpr = `Ini List Pengguna bot Premium \nTotal : ${listprp.length}\n`
                for (let i of listprp) {
                    listpr += `➸ @${i.replace(/@c.us/g,'')}\n`
                }
                client.sendTextWithMentions(from, listpr, id)
                break
            case '!bc2':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
       ...
       case '!mentionall':
        ...
        for (let i = 0; i < groupMem.length; i++) {
            ...
            client.reply(from, `Total Member : ${groupMem.length}`, id)
        ...
        case '!clearall':
            ...
            case '!clearchat':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot', id)
            const allChatz1 = await client.getAllChats()
            for (let dchat of allChatz1) {
        ...
        case '!join':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            //return client.reply(from, 'Jika ingin meng-invite bot ke group anda, silahkan izin ke wa.me/6287883353433', id)
            ...
            if (key !== 'lGjYt4zA5SQlTDx9z9Ca1') return client.reply(from, '*key* salah! silahkan chat owner bot untuk mendapatkan key yang valid, 087883353433.', id)
            const check = await client.inviteInfo(link)
            if (!isLink) return client.reply(from, 'Ini link? 👊🤬', id)
            if (tGr.length > 400) return client.reply(from, 'Maaf jumlah group sudah maksimal!', id)
        ...
        case '!chord':
            ...
            const chord = await get.get(`https://api.i-tech.id/tools/chord?key=3b8JTm-WeSnVm-ynZYWt-mnMT1e-n888YG&query=${query__}`).json()
            if (chord.error) return client.reply(from, chord.error, id)
            client.reply(from, chord.result, id)
            break
        case '!listdaerah':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const listDaerah = await get('http://mhankbarbars.herokuapp.com/daerah').json()
        ...
        case '!jadwalshalat':
            ...
            const jadwalShalat = await get.get(`http://mhankbarbars.herokuapp.com/api/jadwalshalat?daerah=${daerah}`).json()
        ...
        case '!loli':
            ...
            case '!qrgen':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!qrgen [Teks]*', id)
                client.sendFileFromUrl(from, `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${body.slice(7)}`, `qrgen.jpeg`, `Teks : ${body.slice(7)}`, id)
                break 
                case '!randomimg':
                    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.sendFileFromUrl(from, `https://picsum.photos/536/354`, `randomimg.jpeg`, `Nih`, id)
                break  
                case '!randomimg2':
                    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                    await client.sendStickerfromUrl(from, `https://picsum.photos/250/250`, { method: 'get' })
                    break   
                    case '!waifu2':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                        const waifu2 = await get.get(`https://docs-jojo.herokuapp.com/api/waifu`).json() 
                        client.sendFileFromUrl(from, waifu2.image, `waifu2.jpeg`, waifu2.waifu, id)
                        break
                        case '!husbando':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            const husbando = await get.get(`https://docs-jojo.herokuapp.com/api/husbando`).json() 
                            client.sendFileFromUrl(from, husbando.image, `husbando.jpeg`, husbando.waifu, id)
                            break
                            case '!cooltext':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!cooltext [Teks]*', id)
                            const cooltext = await get.get(`https://docs-jojo.herokuapp.com/api/randomcooltext?text=${body.slice(10)}`).json() 
                            client.sendFileFromUrl(from, cooltext.image, `cooltext.jpeg`, cooltext.text, id)
                            break
                            case '!text2img':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!text2img [Teks]*', id)
                            client.sendFileFromUrl(from, `https://assets.imgix.net/examples/clouds.jpg?blur=150&w=640&h=200&fit=crop&txt=${body.slice(10)}&txt-size=62&txt-color=ff2e4357&txt-align=middle,center&txt-font=Futura%20Condensed%20Medium&monochrome=ff6598cc`, `t2i.jpeg`, `nih`, id)
                            break  
                            case '!textsticker':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!textsticker [Teks]*', id)
                                const t2s = `https://assets.imgix.net/examples/clouds.jpg?blur=150&w=250&h=250&fit=crop&txt=${body.slice(13)}&txt-size=62&txt-color=ff2e4357&txt-align=middle,center&txt-font=Futura%20Condensed%20Medium&monochrome=ff6598cc`
                                if (t2s.match(isUrl)) {
                                    await client.sendStickerfromUrl(from, t2s, { method: 'get' })
                                }
                            break 
                            case '!text2sticker':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!text2sticker [Teks]*', id)
                                const ts = await get.get(`http://mhankbarbars.herokuapp.com/api/text2image?text=${body.slice(14)}`).json()
                                await client.sendStickerfromUrl(from, ts.result, { method: 'get' })
                            break
                            case '!ttp':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                           ...
                           case '!party':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!party [Teks]*', id)
                                client.sendFileFromUrl(from, `https://api.vhtear.com/partytext?text=${body.slice(7)}&apikey=${RasiApi}`, `party.jpeg`, `Rasi Tech Bot`, id)
                           break
                           case '!silk':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!silk [Teks]*', id)
                                client.sendFileFromUrl(from, `https://api.vhtear.com/silktext?text=${body.slice(6)}&apikey=${RasiApi}`, `silk.jpeg`, `Rasi Tech Bot`, id)
                           break
                           case '!romantis':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!romantis [nama1] [nama2]*', id)
                                client.sendFileFromUrl(from, `https://api.vhtear.com/romancetext?text=${args[1]}%20dan%20${args[2]}&apikey=${RasiApi}`, `romantis.jpeg`, `Rasi Tech Bot`, id)
                           break
                           case '!texts':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                ...
                                await client.sendImageAsSticker(from, texts.result, { method: 'get' })
                           break
                           case '!ts':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                           ...
                           case '!ts2':
                            ...
                            textToPicture.convert({
                              ...
                              return result.getBase64()
                                ...
                                client.sendImage(from, str) // data:image/png;base64,iVBORw0KGgoA...
                           ...
                           case '!sls':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!sls [Teks]*', id)
                                var mediaData = await decryptMedia(`https://api.vhtear.com/slidingtext?text=${body.slice(5)}&apikey=${RasiApi}`, uaOverride)
                    client.reply(from, '[WAIT] Sedang di proses⏳ silahkan tunggu ± 1 min!', id)
                    var filename = `./media/stickergif.${mimetype.split('/')[1]}`
                    await fs.writeFileSync(filename, mediaData)
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        await client.sendImageAsSticker(from, `data:image/gif;base64,${gif.toString('base64')}`)
                           ...
                           case '!slv':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!slv [Teks]*', id)
                                await client.sendFileFromUrl(from, `https://api.vhtear.com/slidingtext?text=${body.slice(5)}&apikey=${RasiApi}`)
                           break 
                           case '!khs':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!khs [Teks1] [Teks2]*', id)
                                await client.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${args[1]}&text2=${args[2]}&apikey=${RasiApi}`)
                           break 
                           case '!khp':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!khp [Teks1] [Text2]*', id)
                                await client.sendStickerfromUrl(from, `https://api.vhtear.com/pornlogo?text1=${args[1]}&text2=${args[2]}&apikey=${RasiApi}`)
                           break 
                           case '!blackpink':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!blackpink [Teks]*', id)
                                await client.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?text=${body.slice(11)}&apikey=${RasiApi}`, `blackp.jpg`)
                           break 
                           case '!3d':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!3d [Teks]*', id)
                                await client.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/text3d?text=${body.slice(4)}`, `3d.jpg`)
                           break 
                           case '!text':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                           ...
                           case '!glow':
                            ...
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                ...
                                await client.sendFileFromUrl(from, `https://api.vhtear.com/glowtext?text=${body.slice(6)}&apikey=${RasiApi}`, `glow.jpg`)
                           break 
                           case '!love':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!love [Teks]*', id)
                                await client.sendFileFromUrl(from, `https://api.vhtear.com/lovemessagetext?text=${body.slice(6)}&apikey=${RasiApi}`, `love.jpg`)
                           break 
                           case '!hartatahta':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                                if (args.length === 1) return client.reply(from, 'Kirim perintah *!hartatahta [nama]*', id)
                                await client.sendFileFromUrl(from, `https://api.vhtear.com/hartatahta?text=${body.slice(11)}&apikey=${RasiApi}`, `harta.jpg`, `Harta Tahta ${body.slice(11)}`)
                           break 
                            case '!textimg':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                           ...
                           break 
                      //      case '!komiku':
                      //          if (args.length === 1) return client.reply(from, 'Kirim perintah *!komiku [Text]*', id)
                      //          const komiku = await get.get(`http://mhankbarbars.herokuapp.com/api/komiku?q=${body.slice(8)}`).json()
                     //           client.sendFileFromUrl(from, komiku.thumb, `komiku.jpeg`, `Genre : ${komiku.genre}\nInfo : ${komiku.info}\nLink Download : ${komiku.link_dl}\nSinopsis : ${komiku.sinopsis}`, id)
                          //  break 
              //              case '!kusonime':
               //                 if (args.length === 1) return client.reply(from, 'Kirim perintah *!kusonime [Text]*', id)
                //                const kusonime = await get.get(`http://mhankbarbars.herokuapp.com/api/kuso?q=${body.slice(8)}`).json()
                //                client.sendFileFromUrl(from, kusonime.thumb, `kusonime.jpeg`, `Judul : ${kusonime.title}\nInfo : ${kusonime.info}\nLink Download : ${kusonime.link_dl}\nSinopsis : ${kusonime.sinopsis}`, id)
               //             break 
                            case '!koin':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                        ...
                        let coin = listcoin[Math.floor(Math.random() * listcoin.length)]
                                    await client.sendStickerfromUrl(from, coin, { method: 'get' })
                            break 
                            case '!gbk':
                                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                                ...
                                if (args.length === 1) return await client.sendStickerfromUrl(from, gbk, { method: 'get' })
                                if (args[1] === 'g') {
                                    await client.sendStickerfromUrl(from, `https://data5.sticker.fan/stickers_storage/2020/03/24/file_1936554_128x128.png`, { method: 'get' })
                                 } else if (args[1] === 'b') {
                                    await client.sendStickerfromUrl(from, `https://data5.sticker.fan/stickers_storage/2020/03/13/file_1936552_128x128.png`, { method: 'get' })
                                } else if (args[1] === 'k') {
                                    await client.sendStickerfromUrl(from, `https://data5.sticker.fan/stickers_storage/2020/03/24/file_1936553_128x128.png`, { method: 'get' })
                                } else {
                                    await client.sendStickerfromUrl(from, gbk, { method: 'get' })
                                 }
                                 break
                            break 
            case '!waifu':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const waifu = await get.get(`http://mhankbarbars.herokuapp.com/api/waifu?apiKey=${apiKey}`).json()
            client.sendFileFromUrl(from, waifu.image, 'Waifu.jpg', `➸ Name : ${waifu.name}\n➸ Description : ${waifu.desc}\n\n➸ Source : ${waifu.source}`, id)
            break
            case '!jodoh':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!jodoh [nama1] [nama2]*', id)
            const jodoh = await get.get(`https://api.vhtear.com/primbonjodoh?nama=${args[1]}&pasangan=${args[2]}&apikey=${RasiApi}`).json()
            
            client.reply(from, `Hasil : ${jodoh.hasil}`, id)
            break
            case '!color2img':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            case '!mediafire':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (!args[1].includes('mediafire.com')) return client.reply(from, mess.error.Iv, id)
                if (!isLin) return client.reply(from, mess.error.Iv, id)
            const med = await get.get(`http://mhankbarbars.herokuapp.com/api/mdl?url=${args[1]}`).json()
            client.sendFileFromUrl(from, med.link_dl, med.filename, id)
            client.reply(from, `File Name : ${med.filename}\nSize File : ${med.filesize}\nTanggl Upload : ${med.upload_date}`, id)
            break
            case '!wallpaper':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            const wallpaper = await get.get(`http://mhankbarbars.herokuapp.com/api/random/hentai?apiKey=${apiKey}`).json()
            client.sendFileFromUrl(from, wallpaper.result,`walpapper.jpg`, id)
            break
            case '!petir':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (!args[1].includes('rt-files.000webhostapp.com')) return client.reply(from, mess.error.Iv, id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!petir [link gambar]\n\nGambar dapat kalian upload di https://rt-files.000webhostapp.com/*', id)
            const petir = await get.get(`https://api.vhtear.com/lightning?link=${args[1]}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, petir.imgUrl,`petir.jpg`, id)
            break
            case '!textpetir':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!textpetir [text]*', id)
            client.sendFileFromUrl(from, `https://api.vhtear.com/thundertext?text=${body.slice(11)}&apikey=${RasiApi}`,`textpetir.jpg`)
            break
            case '!gsm':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!gsm [nama hp]', id)
            const gsm = await get.get(`https://api.vhtear.com/gsmarena?query=${body.slice(5)}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, gsm.image, `gsm.jpg`, `Judul : ${gsm.title}\n\nSpesifikasi : ${gsm.spec}`, id)
            break
            case '!quran':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!quran [nomor]', id)
            const quranq = await get.get(`https://api.vhtear.com/quran?no=${args[1]}&apikey=${RasiApi}`).json()
            client.reply(from, `Source : ${quranq.source}\n\nSurah : ${quranq.surah}\n\nQuran : ${quranq.quran}`, id)
            break
            case '!hero':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!hero [nama hero] Ex: !hero akai', id)
            const hero = await get.get(`https://api.vhtear.com/herodetail?query=${body.slice(5)}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, hero.pictHero ,`hero.jpg`,`Title : ${hero.title}\n\nQuotes : ${hero.quotes}\n\nInfo : ${hero.info}\n\nAttributes : ${hero.attributes}`, id)
            break
            case '!infomotor':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!infomotor [merek motor] Ex: !infomotor Vario 150', id)
            const infomotor = await get.get(`https://api.vhtear.com/infomotor?merk=${body.slice(11)}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, infomotor.image ,`infomotor.jpg`,`Title : ${infomotor.title}\n\nHarga : ${infomotor.harga}\n\nSpesifikasi : ${infomotor.spesifikasi}\n\nKelebihan : ${infomotor.kelebihan}\n\nKekurangan : ${infomotor.kekurangan}`)
            break
            case '!infomobil':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!infomobil [merek mobil] Ex: !infomotor avanza', id)
            const infomobil = await get.get(`https://api.vhtear.com/infomobil?merk=${body.slice(11)}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, infomobil.image ,`infomobil.jpg`,`Title : ${infomobil.title}\n\nHarga : ${infomobil.harga}\n\nSpesifikasi : ${infomobil.spesifikasi}\n\nKelebihan : ${infomobil.kelebihan}\n\nKekurangan : ${infomobil.kekurangan}`, id)
            break
            case '!valentine':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!valentine [nama1] [nama2] [link gambar1] [link gambar2]\n\nGambar dapat kalian upload di https://rt-files.000webhostapp.com/*', id)
            const valentine = await get.get(`https://api.vhtear.com/valentine?t1=${args[1]}&t2=${args[2]}&l1=${args[3]}&l2=${args[4]}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, valentine.imgUrl, id)
            break
             case '!memegen':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            ...
            if (arg.length >= 3) {
                client.reply(from, mess.wait, id)
                const top = encodeURIComponent(arg[1])
                const down = encodeURIComponent(arg[2])
                const mimage = encodeURIComponent(arg[3])
                if (!args[1].includes('rt-files.000webhostapp.com')) return client.reply(from, mess.error.Iv, id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!memegen [kata1] [kata2] [link gambar]\n\nEx: !memegen |Rasi Tech|Itu Programmer|https://rt.files.000webhostapp.com/asdasdasd.jpg\n\nGambar dapat kalian upload di https://rt-files.000webhostapp.com/*', id)
            const memegen = await get.get(`https://api.vhtear.com/genmeme?text1=${top}&text2=${down}url=${mimage}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, memegen, id)
            }
            break
            case '!profile':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            await client.sendStickerfromUrl(from, profile.large, { method: 'get' })
            client.reply(from, `*Gender :* ${profile.gender}\n\n*Name :* ${profile.name}\n\n*Location :* ${profile.street}\n\n*Email :* ${profile.email}\n\n*login :* ${profile.login}\n\n*Dob :* ${profile.dob}\n\n*Registered :* ${profile.registered}\n\n*Phone :* ${profile.phone}\n\n*Cell :* ${profile.cell}\n\n*ID :* ${profile.id}\n\n*Nat :* ${profile.nat}`,id)
            break
            case '!emoji2img':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!wmoji2img [Kode Emoji]\n\nKode Emoji dapat kalian cari di !emoji')
            
            await client.sendFileFromUrl(from, `https://api.vhtear.com/emojitopng?code=${args[1]}&apikey=${RasiApi}`)
            break
            case '!e2s':
            case '!emostic':
            case '!emoji2stiker':
            ...
            case '!emojistiker':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!e2s [Kode Emoji]* , *!emostic [Kode Emoji]* , *!emoji2stiker [Kode Emoji]* atau *!emojistiker [Kode Emoji]*\n\nKode Emoji dapat kalian cari di !emoji')
                await client.sendStickerfromUrl(from, `https://api.vhtear.com/emojitopng?code=${args[1]}&apikey=${RasiApi}`, { method: 'get' })
            break
            case '!fml':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            case '!kalkulator':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!kalkulator [angka] Ex: !kalkulator 10*3:2+2-5', id)
            const kalkulator = await get.get(`https://api.vhtear.com/calculator?value=${args[1]}&apikey=raaitech`).json()
            client.reply(from, `Hasil : ${kalkulator.data}\n\nInfo : ${kalkulator.info}` , id)
            break
            case '!ramalno':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!ramalno [08xxxx] Ex: !ramalno 087883353433', id)
            const ramalno = await get.get(`https://api.vhtear.com/nomerhoki?no=${args[1]}&apikey=${RasiApi}`).json()
            client.reply(from, ramalno.hasil , id)
            break
            case '!artimimpi':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!artimimpi [mimpi] Ex: !artimimpi ular', id)
            const artimimpi = await get.get(`https://api.vhtear.com/artimimpi?query=${args[1]}&apikey=${RasiApi}`).json()
            client.reply(from, artimimpi.hasil , id)
            break
            case '!jarak':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!jarak [tempat] [tempat] Ex: !jarak jakarta depok', id)
            const jarak = await get.get(`https://api.vhtear.com/distance?from=${args[1]}&to=${args[1]}&apikey=${RasiApi}`).json()
            client.reply(from, jarak.data , id)
            break
            case '!infoalamat':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!infoalamat [tempat] Ex: !infoalamat jalan jerman no 645', id)
            const infoalamat = await get.get(`https://api.vhtear.com/infoalamat?query=${body.slice(12)}&apikey=${RasiApi}`).json()
            client.reply(from, infoalamat.data , id)
            break
            case '!ascii':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!ascii [nama] Ex: !ascii rasi', id)
            const ascii = await get.get(`https://api.vhtear.com/textscreen?query=${args[1]}&apikey=${RasiApi}`).json()
            client.reply(from, ascii.text , id)
            break
            case '!textgif':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!textgif [text] Ex: !textgif rasi ganteng', id)
            await client.sendStickerfromUrl(from, `https://api.vhtear.com/textxgif?text=${body.slice(9)}&apikey=${RasiApi}`, { method: 'get' })
            break
            case '!funkuis':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const funkuis = await get.get(`https://api.vhtear.com/funkuis&apikey=${RasiApi}`).json()
            client.reply(from, `Soal : ${funkuis.soal}\n\nJawaban klik baca selengkapnya\n\n‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎\n\nJawaban : ${funkuis.jawaban}\nDeskripsi : ${funkuis.desk}\nPoin Kuis : ${funkuis.poin}` , id)
            break
            case '!email':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            ...
            client.reply(from, 'Masukan Seperti ini !isiemail test@gmail.com\n\n*!isiemail rasi@gmail.com* email di dapatkan dari !email', id)
        }
            break
            case '!puisi1':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const puisi1 = await get.get(`http://mhankbarbars.herokuapp.com/api/puisi3`).json()
            client.reply(from, puisi1.result , id)
            break
            case '!puisi2':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const puisi2 = await get.get(`http://mhankbarbars.herokuapp.com/api/puisi2`).json()
            client.reply(from, puisi2.result , id)
            break
            case '!puisi3':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const puisi3 = await get.get(`http://mhankbarbars.herokuapp.com/api/puisi1`).json()
            client.reply(from, puisi3.result , id)
            break
            case '!ceritasex':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            const seritasex = await get.get(`http://mhankbarbars.herokuapp.com/api/cersex2`).json()
            client.reply(from, seritasex.result , id)
            break
            case '!cerpen':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const cerpen = await get.get(`http://mhankbarbars.herokuapp.com/api/cerpen`).json()
            client.reply(from, cerpen.result , id)
            break
            case '!seberapabucin':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const sbcn = await get.get(`http://mhankbarbars.herokuapp.com/api/howbucins`).json()
            client.reply(from, `Deskripsi : ${sbcn.desc}\nPersen : ${sbcn.persen}%` , id)
            break
            case '!seberapagay':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const sbgy = await get.get(`http://mhankbarbars.herokuapp.com/api/howgay`).json()
            client.reply(from, `Deskripsi : ${sbgy.desc}\nPersen : ${sbgy.persen}%` , id)
            break
            case '!check':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            client.sendStickerfromUrl(from, yesno.image)
            break
            case '!avatar':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            case '!joox':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!joox [judul]\n\nContoh : *!joox lathi*', id)
                const jux = await get.get(`https://api.vhtear.com/music?query=${body.slice(6)}&apikey=${RasiApi}`).json()
            client.sendFileFromUrl(from, jux.linkImg, `joox.jpg`,`Judul : ${jux.judul}\nAlbum : ${jux.album}\nPenyanyi : ${jux.penyanyi}\nUkuran : ${jux.filesize}\nDurasi : ${jux.duration}\nEkstensi : ${jux.ext}\nLirik : ${jux.lirik}`, id)
            client.sendPtt(from, jux.linkMp3, id)
            break
            case '!sadbot':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            case '!apkmaker':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.sendLinkWithAutoPreview(from, `https://t.me/RasiTechChannel/645`)
                break
                case '!sml':
                    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.sendLinkWithAutoPreview(from, `Sosical Media Lite V23 Final

                Apa Yang Baru?
                                
                - Cek Grub Telegramnya Aja
                                
                ——-Link Download——-
                                
                                
                https://t.me/RasiTechChannel/595
                                
                                
                                
                Gak ZIP kok Jadi Gak Ber Password
                                
                Password : 
                                
                                
                Buat yang gak tau Ini apa
                                
                Ini adalah aplikasi sosical media lite, yang dimana kalian dapat menggunakan semua sosial media dalam 1 Aplikasi dengan ukuran yang kecil 7MB.
                                
                                
                                
                Join Channel @RasiTechChannel Untuk Mengetahui Projek apa saja yang sedang saya kerjakan.`)
                break
            case '!infoner':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            case '!official':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.reply(from, `Join Group Rasi Tech Bot\n\nGroup: https://chat.whatsapp.com/Dd16Q3IRmBuKsFgW1Q0Jr7` , id)
                break
                case '!promosi':
                    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                ...
                case '!gasuka':
                    ...
                    case '!dadu':
                        ...
                        client.sendStickerfromUrl(from, tiker, { method: 'get' })
                        break
                        case '!js':
                            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                            if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            ...
            case '!unblock':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!unblock [628xxxxx]*', id)
                client.contactUnblock(`${args[1]}@c.us`)
                client.reply(from, `Berhasil Unblock ${args[1]}`, id)
            break
            case '!block':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot', id)
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!unblock [628xxxxx]*', id)
                client.contactBlock(`${args[1]}@c.us`)
                client.reply(from, `Berhasil Block ${args[1]}`, id)
            break
            case '!darkmode':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot', id)
                  if (args[1].toLowerCase() === 'enable') {
                    if (args[1] === `enable`) returnclient.darkMode(true)  
                    client.reply(from, `Berhasil Mengaktifkan Dark Mode Bot`, id) 
                } else if (args[1].toLowerCase() === 'disable') {
                    if (args[1] === `enable`) returnclient.darkMode(true)  
                    client.reply(from, `Berhasil Menonaktifkan Dark Mode Bot`, id) 
                } else {
                    client.reply(from, 'Pilih enable atau disable udin!', id)
                }
                break
            ...
            case '!bapack':
                ...
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!bapack [Text]*', id)
                    ...
                    case '!tebakgambaer':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                    const tebakgambaer = await get.get(`https://api.vhtear.com/tebakgambar&apikey=${RasiApi}`).json()
                    client.sendFileFromUrl(from, tebakgambaer.soalImg ,`tebakgambar.jpg`,`Poin : ${tebakgambaer.poin}`, id)
                    
                    
                        client.reply(from, `Jawaban\n\n‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎\n${tebakgambaer.jawaban}` , id)
                    
                   
                    break
                    case '!ttslontong':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                        client.repl(from, `Perintah di ganti menjadi *!funkuis*`, id)
                        break
                    case '!search':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                ...
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!smart [no]*', id)
                    ...
                    const simi = await get.get(`https://st4rz.herokuapp.com/api/simsimi?kata=${body.slice(6)}`).json()
                    client.reply(from, simi.result , id)
                    break
                    case '!spamcall':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                        const spamcall = await get.get(`https://rasi-tech-api.000webhostapp.com/spam.php?apikey=rasitech&no=${args[1]}`).json()
                        client.reply(from, spamcall.message , id)
                    break
                    case '!spamcall2':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                if (!isGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                const groupMem8 = await client.getGroupMembers(groupId)
                 for (let i = 0; i < groupMem8.length; i++) {
                   var nomory = `${groupMem8[i].id.replace(/@c.us/g, '').replace(/@/g, '').replace(body.slice(2),'')}`
                   client.sendText(nomory)
                   //const scall2 = await get.get(`http://mhankbarbars.herokuapp.com/api/spamcall?no=${nomory_s}`).json()
                   //client.reply(from, scall2.logs , id)
                 }
                //client.reply(from, `Sukses spam call member` , id)
                break
                    case '!spasms':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                        if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                        if (args.length === 1) return client.reply(from, 'Kirim perintah *!spamsms [nomor ex 6285764424816] [jumlah ex 10]*', id)
                    const ssms = await get.get(`http://mhankbarbars.herokuapp.com/api/spamsms?no=${args[1]}&jum=${args[2]}`).json()
                    client.reply(from, ssms.logs , id)
                    break
                    case '!katahilih':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                        if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
            ...
            if (arg.length >= 3) {
                client.reply(from, mess.wait, id)
                const k1w = encodeURIComponent(arg[1])
                const k2w = encodeURIComponent(arg[2])
                const katawarna = await get.get(`https://api.vhtear.com/watercolour_text?text1=${k1w}&text2=${k2w}&apikey=${RasiApi}`).json()
                    client.sendFileFromUrl(from, katawarna.imgUrl ,`katawarna.jpg`, `Wow Astetik`, id)
                
            } else {
                client.reply(from, 'Masukan Seperti ini !katawarna |teks1|teks2|\n\n*!katawarna |harta tahta|rasi tech|*\n\nGaris | yang berada di tengah itu sebagai spasi.', id)
            }
            break
            case '!pastebin':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            if (arg.length >= 3) {
                ...
                const pastebin = await get.get(`https://rt-files.000webhostapp.com/pastebin.php?apikey=rasitech&privasi=0&judul=${judulp}&teks=${textp}`).json()
            ...
            if (arg.length >= 3) {
                client.reply(from, mess.wait, id)
                const g1w = encodeURIComponent(arg[1])
                const g2w = encodeURIComponent(arg[2])
                    client.sendFileFromUrl(from, `https://api.vhtear.com/glitchtext?text1=${g1w}&text2=${g2w}&apikey=${RasiApi}` ,`Glitch.png`, `Glitch`)
                
            } else {
                client.reply(from, 'Masukan Seperti ini !glitch |teks1|teks2|\n\n*!glitch |Rasi Tech|Subcribe|*\n\nGaris | yang berada di tengah itu sebagai Enter.', id)
            }
            break
            case '!google':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            if (arg.length >= 4) {
                client.reply(from, mess.wait, id)
                const google1 = encodeURIComponent(arg[1])
                const google2 = encodeURIComponent(arg[2])
                const google3 = encodeURIComponent(arg[3])
                    client.sendFileFromUrl(from, `https://api.vhtear.com/googletext?text1=${google1}&text2=${google2}&text3=${google3}&apikey=${RasiApi}` ,`google.jpg`, `Kata Kunci : ${google1}\nHasil 1 : ${google2}\nHasil 2 : ${google3}`, id)
                
            } else {
                client.reply(from, '!google |Rasi Ganteng|Cara Menjadi Rasi|Rasi Tech Keren|', id)
                client.sendFile(from, './media/img/google.png', 'google.png', `Kata Kunci : Rasi Ganteng\nHasil 1 : Cara Menjadi Rasi\nHasil 2 : Rasi Tech Keren`)
            }
            break
            case '!teks':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            if (arg.length >= 3) {
                if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
                client.reply(from, mess.wait, id)
                const t1w = encodeURIComponent(arg[1])
                const t2w = encodeURIComponent(arg[2])
                    client.sendFileFromUrl(from, `https://api.vhtear.com/textmaker?text=${t1w}&warna=${t2w}&apikey=${RasiApi}` ,`teks.jpg`, `Wow Keren`, id)
                
            } else {
                client.reply(from, 'Masukan Seperti ini !teks |teks1|warna|\n\n*!teks |Rasi Tech Ganteng|green|*.', id)
            }
            break
                    case '!qts':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            case '!images':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (args.length == 0) return client.reply(from, `Masukan seperti ini !images kucing`, id)
            const images = await get.get(`https://api.vhtear.com/googleimg?query=${body.slice(8)}&apikey=${RasiApi}`).json()
            const res_images = images.result_search
            const rimages = res_images[Math.floor(Math.random() * res_images.length)]
            client.sendFileFromUrl(from, rimages, 'images.png', `Rasi Tech Bot\n\nHasil Dari *${body.slice(8)}*.`)
                break
                case '!infosurah':
                    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
        ...
        case '!alaudio':
              ...
              bhs = ""
                ...
                if(!isNaN(nmr)) {
                  ...
                  if(isNaN(ayat)) {
                    ...
                    var {names, name_translations, number_of_ayah, number_of_surah,  recitations} = responsih2.data
            ...
            case '!apiesamsat':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.sendFile(from, `API-ESAMSAT.pdf`,`API-ESAMSAT.pdf` , id)
                break
              //  case '!cekno':
         //           if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
         //           if (!isPr) return client.reply(from, 'Perintah ini Khusus Member Premium', id)
          //          if (args.length === 1) return client.reply(from, 'Kirim perintah *!cekno [628xxx / 08xxxx]*', id)
         //   const cekno = await get.get(`https://docs-jojo.herokuapp.com/api/phonenumber?no=${args[1]}`).json()
        //    client.reply(from, `International : ${cekno.international}\nNational : ${cekno.national}\nOperator : ${cekno.carrier}\nType : ${cekno.type}\nLokasi : ${cekno.location}\nTimezone : ${cekno.timezones}` , id)
            break
            case '!ip':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
        ...
        case '!randomnekonime':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const nekonime = await get.get('http://mhankbarbars.herokuapp.com/api/nekonime').json()
        ...
        case '!url2img':
            ...
            const url2img = await get.get(`http://mhankbarbars.herokuapp.com/api/url2image?url=${_query}`).json()
            ...
            case '!celah':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const _query3 = body.slice(7)
            if (!_query3.match(isUrl)) return client.reply(from, mess.error.Iv, id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!url2img [web]*\nContoh *!url2img https://google.com*', id)
            const celah = await get.get(`http://mhankbarbars.herokuapp.com/api/url2image?url=${_query3}/robots.txt`).json()
            if (celah.error) return client.reply(from, celah.error, id)
            client.sendFileFromUrl(from, celah.result, `hasil.jpg`)
            break
            case '!groupicon':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const _query2 = body.slice(11)
            if (!_query2.match(isUrl)) return client.reply(from, mess.error.Iv, id)
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!groupicon [linkimage]*\nContoh *!groupicon https://google.com/blabla.jpg*', id)
            client.setGroupIconByUrl(GroupId, _query2)
            break
        case '!quote':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
        case '!quotes':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            const quotes = await get.get('http://mhankbarbars.herokuapp.com/api/randomquotes').json()
        ...
        case '!help':
            ...
            case '!stop':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                if (isGroupMsg) return client.reply(from, `Perintah ini tidak bisa di group!`)
                client.deleteChat(from)
            break
            case '!app':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.reply(from, `Download Aplikasi Untuk Add Bot Ke Group Kalian\n\nhttp://www.mediafire.com/file/rpdqn231hn8ih02/Admin_bot_wa+(4).apk/file`, id)
            break
            case '!game':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                client.reply(from, `Download game dengan nama *Ngeselin* yang di buat oleh rasi\n\nLink Download : http://www.mediafire.com/file/u8zl7dgsa7x9ysq/ngeselin+(1).apk/file`, id)
            break
            case '!groupc':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.reply(from, `☾ Group Commands ☽

➥1. *!add 62858xxxxx*
Menambahkan nomor ke group
Contoh : !add 6287883353433

➥2. *!kick @tagmember*
Mengeluarkan Member dari group
Contoh : !kick @tagmember

➥3. *!promote @tagmember*
Menjadikan member sebagai admin 
Contoh : !promote @tagmember

➥4. *!demote @tagadmin*
Memberhentikan member sebagai admin
Contoh : !demote @tagmember
...
➥8. *!leave*
Mengeluarkan bot dari group
Contoh : !leave

➥9. *!linkGroup*
Mendapatkan link group
Contoh : !linkgroup

➥10. *!delete [replyChatBot]*
Menghapus pesan bot di group
Contoh : !delete [Tag Pesan / Reply pesan]

➥11. *!kickAll*
Mengeluarkan semua member dari group
Contoh : !kickall

➥12. *!welcome [enable|disable]*
Mengaktifkan fitur welcome di group
Contoh : !welcome enable

➥13. *!grouptitle [text]*
Merubah judul group
Contoh :  !grouptitle [Judulnya]

➥14. *!groupchat [enable|disable]*
Membuka / Menutup group
Contoh : !groupchat disable

➥15. *!groupedit [enable|disable]*
Mengaktifkan / Menonaktifkan siapa saja yang bisa mengedit info group
Contoh : !groupedit disable

➥16. *!groupicon [linkgambar]*
Merubah icon group dari link
Contoh : !groupicon https://rt-files.000webhostapp.com/files/pp.jpg

➥17. *!gantilink*
Mengganti link group dengan yang baru
Contoh : !gantilink
...
Contoh : !tutup`, id)
            ...
            case '!downloadc':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.reply(from, `☾ Downloader Commands ☽ [!Beta]
➥1. *!ytmp3 [linkYt]*
Mendownload video youtube dalam bentuk musik
Contoh : !ytmp3 https://youtu.be/linknya

➥2. *!ytmp4 [linkYt]*
Mendownload video youtube
Contoh : !ytmp4 https://youtu.be/linknya

➥3. *!ig [linkIg]*
Mendownload Media instagram
Contoh : !ig [linkig]

➥4. *!fb [linkFb]*
Mendownload Video Facebook
Contoh : !fb [linkfb]

➥5. *!twitter [linkTwitter]*
Mendownload video twitter
Contoh : !twitter

➥6. *!tiktok [LinkTiktok]*
Mendownload Video tiktok
Contoh : !tiktok [LinkTiktok]

➥7 *!ttkmp3 [IDtiktok]*
Belum ada

➥8. *!tkmp4nwm [IDtiktok] <-- No Watermark*
Belum ada`, id)
            break
            case '!otherc':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.reply(from, `☾ Others Commands ☽
➥9. *!sticker*
Gambar ke Stiker
Contoh : !sticker / !stiker [Tag gambar / di captionnya]

➥10. *!stickerGif*
Video / Gif ke Sticker
Contoh : !stickergif / !stikergif / !sgf [Tag \ di captionnya]
...
➥15. *!randomimg*
Random gambar internet
Contoh : !randomimg

➥16. *!fml*
Random kata-kata orana yang benci hidup
Contoh : !fml

➥17. *!apiesamsat*
Dokumentasi API Esamsat
Contoh : !apiesamsat
...
➥19. *!cekno [628xxxx]*
Mendapatkan informasi nomor
Contoh : !cekno [62xxx / 08xxx]

➥20. *!tampar [@tagmember]*
Tampar member
Contoh : !tampar @tagmember

➥21. *!harinasional*
Mendapatkan hari nasional
Contoh : !harinasional [tanggal]

➥22. *!randomimg2*
Random gambar dalam size sticker
Contoh : !randomimg2

➥23. *!randomstic*
Random gambar dalam bentuk sticker
Contoh : !randomstic

➥24. *!corona [Walau Hanya Konspirasi :P]*
Mendapatkan informasi korona dunia
Contoh : !corona
...
➥27. *!katagaul*
Mendapatkan kata gaul hari ini
Contoh : !katagaul

➥28. *!randomtongue*
Mendapatkan random tongue
Contoh : !randomtongue

➥29. *!jadwalShalat [daerah]*
Memunculkan jadwal sholat
Contoh : !jadwalsholat [darahmu]

➥30. *!jadwalTv [channel]*
Jadwal TV hari ini
Contoh : !jadwaltv [channel]
...
➥32. *!listdaerah*
List daerah yang support di bot
Contoh : !listdaerah
...
➥34. *!cuaca [tempat]*
Mendapatkan informasi cuaca di darah kamu
Contoh : !cuaca

➥35. *!tts [kode bhs] [teks]*
Teks ke suara
Contoh : !tts [kode bahsa] [teks]

➥36. *!igStalk [@username]*
Stalking akun instagram
Contoh : !igstalk

➥37. *!twitstalk [username]*
Stalking akun twitter
Contoh : !twitstalk
...
➥40. *!brainly [pertanyaan] [.jumlah]*
Bertanya kepada brainly
Contoh : !brainly [pertanyaan] [.jumlah]
...
➥48. *!getses*
Mendapatkan screenshot server
Contoh : !getses

➥49. *!listblock*
List nomor yang di blokir bot
Contoh : !listblock

➥50. *!nh*
Mendapatkan kode nuclear
Contoh : !nh [code]
...
➥52. *!text2img [width] [heigh] [blur] [text]*
Custom text ke gambar
Contoh : !text2img
...
➥61. *!info*
Informasi Sosial media developer
Contoh : !info
...
➥64. *!artinama*
Arti nama
Contoh : !artinama [udin]

➥65. *!quotemaker [|teks|author|theme]*
Membuat quotes
Contoh : !quotemaker |Jadilah seperti larry|Rasi Tech|laut

➥66. *!join [linkGroup]*
Add bot ke group kamu
Contoh : !join [link group]
...
➥70. *!wait*
Mencari anime dari gambar
Contoh : !wait [tag gambar anime / reply]
...
➥73. *!color2img*
Warna ke gambar
Contoh : !color2img [kode warna]

➥74. *!koneksibot*
Terganti menjadi P / Ping

➥75. *!url2img*
Screenshot Website
Contoh : !url2img https://www.google.co.id/
...
➥78. *!donasi*
Berdonasi ke pada owner
Contoh : !donasi

➥79. *!linkwa [62xxxx]*
Nomot ke link
Contoh : !linkwa [6287xxxx]

➥80. *!lirik [optional]*
Mencari Lirik lagu
Contoh : !lirik armada asal kau bahagia

➥81. *!chord [query]*
Mencari chord gitar
Contoh : !chord selalu ada
...
Belum bisa
➥90. *!suka [Namamu]*
Tanggapan kamu terhadap bot
Contoh : !suka [nama]

➥91. *!gasuka [Namamu]* <-- Tanggapan Anda Terhadap Bot
Tanggapan kamu terhadap bot
Contoh : !gasuka [nama]

➥92. *!kerangajaib*
Kerang ajaib
Contoh : !kerangajaib [pertanyaam]
...
➥94. *!APKMaker*
Membuat aplikasi tanpa komputer
Contoh : !apkmaker

➥95. *!readmore*
Membuat tombol baca selengkapnya walau teks pendek
Contoh : !readmore |teks1|teks2|

➥96. *!sml [Sosial Media Lite]*
Aplikasi Sosial media lite Versi Terbaru, Semua sosial media dalam 1 aplikasi dengan size 7MB
Contoh : !sml

➥97. *!bapack [Kata]*
Teks ke font bapak-bapak
Contoh : !bapack [kata]

➥98. *!jodoh [nama1] [nama2]*
Jodoh nama1 dengan nama2
Contoh : !jodoh [nama1] [nama2]

➥99. *!pesan [pesan]*
Pesan anda akan terkirin ke owner dan jika anda spam maka bot akan blokir anda!
Contoh : !pesan [pesan]
...
➥105. *!celah [Url Website]*
Mencari celah dari website
Contoh : !celah https://www.google.co.id

➥106. *!chatid*
Memunculkan ID kontak kamu di bot
Contoh : !chatid

➥107. *!reportspam [62xxxx]*
Mreport Nomor Via bot
Contoh : !reportspam
...
➥110. *!kataalay [text]*
Membuat kata alay
Contoh : !kataalay [teks]

➥111. *!katahilih [text]*
Merubah teks biasa menjadi hilih
Contoh : !katahilih Halah konci

➥112. *!kataninja [text]*
Merubah kata biasa menjadi kata ninja
Contoh : !kataninja Apa kabar?

➥113. *!nama [laki | cewe]*
Random nama laki / cewe
Contoh : !nama laki

➥114. *!kambing*
Random gambar kambing
Contoh : !kambing

➥115. *!katawarna [Text]*
Teks warna
Contoh : !katawarna |kata1|kata2|
...
➥119. *!md5 [Text]*
Meruabah Teks menjadi md5
Contoh : !md5 admin

➥120. *!tkstalker [@rasi_tech]*
Masih Pengembangan

➥121. *!nulis2 [teks]*
Menulis di buku
Contoh : !nulis2 apa kabar? 1+1=2
...
➥138. *!game*
Download Game *Ngeselin* Buatan Rasi Tech
Contoh : !game
...
➥145. *!kusonime [anime]*
Mengambil kusonime
Contoh : !kusonime [anime]

➥146. *!mediafire [link]*
Link mediafire ke dokumen wa
Contoh : !mediafire [link mediafire]
...
➥149. *!wallpper*
Mengambil Wallpaper Anime
Contoh : !wallpaper

➥150. *!twitter*
Mendownload Video Twitter
Contoh : !twitter [link twitter]

➥151. *!twitstalk*
Masih Pengembangan

➥152. *!terjemah [en] [kata]*
Menerjemahkan kata
Contoh : !terjemah [kode bahasa] [kata]

➥153. *!cogan*
Mengambil Cowo Ganteng Lokal
Contoh : !cogan

➥154. *!images [kata]*
Mencari Gambar lewat bot
Contoh : !images kucing

➥155. *!listsurah*
Mendapatkan list surah2 al-quran
Contoh : !listsurah
...
➥163. *!kalkulator*
menghitung angka
Contoh : !kalkulator 1+1

➥164. *!infomotor*
Mendapatkan informasi motor2
Contoh : !infomotor vario 150

➥165. *!infomobil*
Mendapatkan info mobil2
...
➥171. *!ramalno*
Meramal nomor hp seseorang
Contoh : !ramalno 085764424816

➥172. *!artimimpi*
Mengetahui arti mimpi
Contoh : !artimimpi ular

➥173. *!cek*
Mengecek apakah nomor tersebut terdaftar di WhatsApp apa tidak
Contoh : !cek 6287883353433
...
➥176. *!tod*
Random Soal Truth Or Dare
Contoh : !tod

➥176. *!code*
Code Editor Via Bot
Contoh : !code <h1>RasiTech</h1>

➥176. *!see*
Melihat codingan kita di !code
Contoh : !see

➥176. *!wa*
Random WhatsApp Group Generator
Contoh : !wa

➥176. *!tts2*
Text to Sound Laki
Contoh : !tts2 Apa Kabar

➥176. *!avatar [gender] [nama]*
membuat afatar
Contoh : !avatar male rasi
...
Contoh : !yesno`, id)
            ...
            case '!seruc':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.reply(from, `☾ Seru-Seruan Commands ☽
➥1. *!kusuka [@tagmember]*
Anda Suka Dengan @tagmember
Contoh : !kusuka @tagmember

➥2. *!peluk [@tagmember]*
Memeluk Orang yang anda suka
Contoh : !peluk @tagmember

➥3. *!cium [@tagmember]*
Mencium orang yang anda suka
Contoh : !cium @tagmember

➥4. *!tampar [@tagmember]*
Menampar orang yang anda kesal
Contoh : !tampar @tagmember

➥5. *!keroyok [@tagmember]*
Mengeroyok anggota group
Contoh : !keroyok @tagmember

➥6. *!ruqiyah [@tagmember]*
Meruqiyah member
Contoh : !ruqiyah @tagmember

➥7. *!loveu [@tagmember]*
Memberika love You kepada seseorang
Contoh : !loveu @tagmember
...
➥9. *!ngopi*
Mengambil gambar kopi
Contoh : !ngopi

➥10. *!gelaskopi*
Mengambll gambar kopi v2
Contoh : !gelaskopi

➥11. *!kopi [Pesan Ajakan]*
Mengajak orang buat ngopi
Contoh : !kopi ngopi slur

➥12. *!kutuk [@tagmember] [emoji/kutukan]*
Mengutuk member #bercanda
Contoh : !kutuk @tagmeber batu

➥13. *!kabur*
Kabur bersama bot
Contoh : !kabur

➥14. *!doa [@tagmember] [Pesan doa]*
Mendoakan member
Contoh : !doa @tagmember sehat

➥15. *!bogem [@tagmember]*
Membogem member
Contoh : !bogem @tagmember

➥16. *!nasa*
Suka Terjadi Kesalahan
...
➥22. *!pantunpakboi*
Mendapatkan pantun fakboi
Contoh : !pantunpakboi

➥23. *!oranghilang [LinkImage] [nomor/nama] <- tag [nama]*
Membuat gambar orang hilang
Contoh : !oranghilang [link gambar] [nama/nomor] [nama yang hilang]

➥24. *!calendargen [UrlImage]*
Bikin kalender
Contoh : !calendergen

➥22. *!seberapagay*
Seberapa gay kah kamu?
Contoh : !seberapagay

➥22. *!seberapabucin*
Seberapa bucin kah kamu?
Contoh : !seberapabucin`, id)
            break
            case '!dewac':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.reply(from, `☾ Dewa Commands Admin☽
➥1. *!spam [nomor] [jumlah] [pesan]*
Spam ke sesama pengguna bot
Contoh : !spam [nomor] [jumlah ex 10] [halo]

➥2. *!mentionall2*
Mention member secara terpisah
Contoh : !mentionall2

➥3. *!repeate [jumlah max 100] [line/noline] [kata]*
Mengul kalimat
Contoh : !repeate [100] line Rasi

➥4. *!mentionall3*
Mengambil semua nomor member
Contoh : !mentionall3

➥5. *!bcmem [Pesan]*
Broadcast ke semua member
Contoh : !bcmem [pesan] <- Berlaku kita member sudah chat robot

➥6. *!linkmem*
Mendapatkan Link WA semua member
Contoh : !linkmem
...
Belum bisa`, id)
            ...
            case '!ownerc':
                if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.reply(from, `☾ Owner Commands ☽
➥1. *!unblock [nomor]*
Unblock pengguna bot
Contoh : !unblock 62xxx
...
➥3. *!posttxt [Text]*
Memposting teks ke status bot
Contoh : !posttxt Testing nih
...
➥5. *!clearAll*
Menghapus semua chat seingga akan seperti baru register wa
Contoh : !clearall

➥6. *!DarkMode*
Mengaktifkan dark mode di Getses
Contoh : !darkmode enable

➥7. *!clearchat*
Membersihkan semua percakapan member
Contoh : !clearchat`, id)
break
case '!premiumc':
    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
client.reply(from, `☾ Premium Commands ☽
➥1. *!faced*
Deteksi muka (umur,kelamin,marah dll)
Contoh : !faced [linkGambar]

➥2. *!sms*
Mengirim sms ke orang tanpa pulsa
Contoh : !sms |halo apa kabar rasi ganteng?|08xxxx

➥3. *!stickernobg [caption]*
Membuat sticker tanpa background
Contoh : !stickernobg [Caption]

➥3. *!sticker2img / !stiker2img / !simg [tag sticker]*
Mengubah sticker ke gambar
contoh : !simg [tag stiker yang mau dijadikan gambar]

➥4. *!joox [lagu]*
Mengambil lagu dari joox
Contoh : !joox lathi

➥5. *!smule [LinkSmule]*
Mendownload Smule
Contoh : !smule [linksemule]
...
➥7. *!katawarna [kata]*
Membuat kata warna
Contoh : !katawarna |rasi|tech|

➥8. *!katalay [kata]*
Membuat kata alay
Contoh : !kataalay hallo gaes
...
➥10. *!kataninja [kata]*
Membuat kata ninja
Contoh : !kataninja hallo rasi

➥11. *!katapasir [kata]*
Menulis di pasir pantai
Contoh : !katapasir Rasi Tech

➥12. *!spamcall [8xxxxx] <- Dimulai dari angka 8*
Menelefon seseorang lewat bot
Contoh : !spamcall

➥13. *!spamsms [nomor ex 62xxxx] [jumlah ex 10]*
Spam sms ke nomor
Contoh : !spamsms 62xxxx 10

➥14. *!stop*
Memberhentikan BC Bot

➥15. *!spamcall2*
belum bisa

➥16. *!email*
Mendapatkan Email Tidak terpakai untuk register
Contoh : !email
...
➥18. *!ttp [text]*
Text to picture
Contoh : !ttp Halo gaes

➥19. *!texts [text]*
Text to sticker
Contoh : !texts Halo gaes

➥20. *!valentin*
Membuat Foto Valentine
Contoh : !valentine [nama1] [nama2] [urlimage1] [urlumage2]

➥21. *!memegen*
Membuat meme
Contoh : !meme [kata atas] [katabawah] [linkgambar]
...
➥25. *!textgif* 
Text warna-warni sticker
Contoh : !textgif [kata]

➥26. *!petir* 
Gambar ke petir
Contoh : !petir [linkgambar]

➥27. *!jarak* 
Jarak lokasi
Contoh : !jarak depok jakarta

➥28. *!infoalamat* 
Informasi alamat
Contoh : !infoalamat jakarta

➥29. *!backpink* 
Membuat tulisan blackpink
Contoh : !blackpink Rasi Ganteng

➥30. *!textpetir*
Membuat tulisan petir
Contoh : !textpetir Rasi Ganteng

➥31. *!img2fire* 
Gambar ke api
Contoh : !img2fire [linkGambar]
...
➥34. *!hartatahta*
Membuat tulisan harta tahta
Contoh : !hartatahta Rasi

➥35. *!ddos*
DDOS Situs
Contoh : !ddos [jumlah] [website]

➥36. *!mtd*
Media to dokumen
Contoh : !mtd [tag/caption]

➥37. *!vts*
Video to Sound
Contoh : !vts [tag/caption]

➥38. *!spotify*
Mendapatkan akun premium spotify
Contoh : !spotify

➥39. *!netflix*
Mendapatkan akun premium netflix
Contoh : !netflix

➥40. *!cc*
Mendapatkan kartu kredit gratis
Contoh : !cc

➥41. *!akun*
Mendapatkan akun email gratis
Contoh : !akun
...
➥48. *!khs*
Teks Kuning Hitam Sticker
Contoh : !khs rasi tech

➥49. *!khp*
Teks Kuning Hitam Picture
Contoh : !khp rasi tech

➥50. *!emoji2img*
emoji to Image
Contoh : !emoji2image [kodei emoji]
...
➥52. *!shopee*
Mencari Produk Shopee
Contoh : !shopee batrai

➥53. *!e2s !emoji2stiker !emojistiker*
emoji to Sticker
Contoh : !e2s [kodei emoji]

➥54. *!google*
Membuat Teks Google
Contoh : !google |Rasi Ganteng|Cara Menjadi Rasi|Rasi Tech Keren| Untuk contoh hasil kalian dapat mengetikan *!google* tanpa teks apapun

➥55. *!party [Teks]*
Membuat Teks Party
Contoh : !party Rasi Ganteng

➥56. *!romantis [nama1] [nama2]*
Membuat Gambar romantis
Contoh : !romantis Rasi Aulia

➥57. *!silk [Teks]*
Membuat tulisan Silk
Contoh : !silk Rasi Ganteng
...
➥58. *!glitch*
Membuat Tulisan Glitch
Contoh : !glitch |Rasi Tech|Subcribe|

➥58. *!love*
Membuat gambar Love
Contoh : !love Rasi Tech

➥58. *!glow*
Membuat tulisan glow
Contoh : !glow Rasi Tech

➥58. *!ssticker*
Mencari Sticker Online
Contoh : !ssticker Lucu
...
➥60. *!pastebin*
Nulis di Pastebin
Contoh : !pastebin |Judul|Tulisannya|`, id)
            break
        case '!readme':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
        ...
        case '!info':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            client.sendText(from, 'Follow me on Social Media\n\nWhatsApp : 08788335433\nInstagram : @Rasi_Tech\nYoutube : Rasi Tech (ada dua channel)\nTelegram : @rasirt\nTikTok : Rasi_Tech\nPartner WA : +1 (516) 577-0838')
            break
